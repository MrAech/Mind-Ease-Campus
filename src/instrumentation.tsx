...
async function reportErrorToMonitoring(errorData: {
  error: string;
  stackTrace?: string;
  filename?: string;
  lineno?: number;
  ...
  console.error("Reported error:", { ...errorData, url: window.location.href });

  const monitoringUrl = import.meta.env.VITE_MONITORING_URL;
  const monitoringAppId = import.meta.env.VITE_MONITORING_APP_ID;

  if (!monitoringUrl || !monitoringAppId) {
    return;
  }
  ...
  try {
    await fetch(monitoringUrl, {
      ...
      body: JSON.stringify({
        ...
        projectSemanticIdentifier: monitoringAppId,
      }),
    });
  } catch (error) {
    console.error("Failed to report error to monitoring endpoint:", error);
  }
}
  ...
  return (
    ...
    >
      <DialogContent className="bg-red-700 text-white max-w-4xl">
        ...
        <DialogFooter>
          {import.meta.env.VITE_MONITORING_APP_ID ? (
            <a
              href={`https://example-monitoring.example/project/${import.meta.env.VITE_MONITORING_APP_ID}`}
              target="_blank"
            >
              <Button>
